...
func main() {
	...
	reqParams := &app.GetAppQRCodeRequest{
		AccessToken: "#token", // 文档中对应字段：access_token，实际使用时请替换成真实参数
		Path:        "",       // 文档中对应字段：path，实际使用时请替换成真实参数
		PackageID:   "",       // 文档中对应字段：package_id，实际使用时请替换成真实参数
		Width:       465,      // 文档中对应字段：width，实际使用时请替换成真实参数
	}

	resp, err := app.GetAppQRCode(reqParams)
	if err != nil {
		...
		if resp.ContentType == utils.ConverterTypePNG {
			fileName := "./GetAppQRCodeRequest.png"
			_ = ioutil.WriteFile(fileName, resp.Data, 0644)
		}
		fmt.Printf("%#v\n", resp)
	}
}
