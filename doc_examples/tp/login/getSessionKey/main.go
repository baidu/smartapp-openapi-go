...
func main() {
	...
	reqParams := &login.GetSessionKeyRequest{
		AccessToken: "#token",             // 文档中对应字段：access_token，实际使用时请替换成真实参数
		Code:        "",                   // 文档中对应字段：code，实际使用时请替换成真实参数
		GrantType:   "authorization_code", // 文档中对应字段：grant_type，实际使用时请替换成真实参数
	}

	resp, err := login.GetSessionKey(reqParams)
	if err != nil {
