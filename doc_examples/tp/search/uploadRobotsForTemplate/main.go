...
func main() {
	...
	uploadRobotsForTemplateRequestFileVal := search.UploadRobotsForTemplateRequestFile{
		Name:   "robots",
		Reader: fileReader,
	}
	reqParams := &search.UploadRobotsForTemplateRequest{
		AccessToken: "#token",                              // 文档中对应字段：access_token，实际使用时请替换成真实参数
		TemplateID:  185,                                   // 文档中对应字段：template_id，实际使用时请替换成真实参数
		Robots:      uploadRobotsForTemplateRequestFileVal, // 文档中对应字段：robots，实际使用时请替换成真实参数
	}

	resp, err := search.UploadRobotsForTemplate(reqParams)
	if err != nil {
