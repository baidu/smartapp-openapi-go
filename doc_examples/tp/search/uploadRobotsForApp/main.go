...
func main() {
	...
	uploadRobotsForAppRequestFileVal := search.UploadRobotsForAppRequestFile{
		Name:   "robots",
		Reader: fileReader,
	}
	reqParams := &search.UploadRobotsForAppRequest{
		AccessToken: "#token",                         // 文档中对应字段：access_token，实际使用时请替换成真实参数
		Robots:      uploadRobotsForAppRequestFileVal, // 文档中对应字段：robots，实际使用时请替换成真实参数
	}

	resp, err := search.UploadRobotsForApp(reqParams)
	if err != nil {
