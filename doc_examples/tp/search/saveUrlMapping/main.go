...
func main() {
	...
	reqParams := &search.SaveURLMappingRequest{
		AccessToken:  "#token",                                                                                                                                                                           // 文档中对应字段：access_token，实际使用时请替换成真实参数
		Content:      "[{\"h2Pattern\":\"http://www.baidu.com/(d+).html\",\"appPattern\":\"/path/abc/${5}\",\"urlPairs\":[{\"h0Url\":\"http://www.baidu.com/030.html\",\"appUrl\":\"/page/abc/706\"}]}]", // 文档中对应字段：content，实际使用时请替换成真实参数
		CreateMethod: "17",                                                                                                                                                                               // 文档中对应字段：create_method，实际使用时请替换成真实参数
	}

	resp, err := search.SaveURLMapping(reqParams)
	if err != nil {
