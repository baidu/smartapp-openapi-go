...
func main() {
	...
	uploadImageRequestFileVal := image.UploadImageRequestFile{
		Name:   "multipartFile",
		Reader: fileReader,
	}

	reqParams := &image.UploadImageRequest{
		AccessToken:   "#tpToken",                // 文档中对应字段：access_token，实际使用时请替换成真实参数
		MultipartFile: uploadImageRequestFileVal, // 文档中对应字段：multipartFile，实际使用时请替换成真实参数
		Type:          2,                         // 文档中对应字段：type，实际使用时请替换成真实参数
	}

	resp, err := image.UploadImage(reqParams)
	if err != nil {
