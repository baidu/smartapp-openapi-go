...
func main() {
	...
	reqParams := &feed.DeleteResourceRequest{
		AccessToken: "#token",                         // 文档中对应字段：access_token，实际使用时请替换成真实参数
		AppID:       185,                              // 文档中对应字段：app_id，实际使用时请替换成真实参数
		Path:        "/pages/detail/detail?id=567830", // 文档中对应字段：path，实际使用时请替换成真实参数
	}

	resp, err := feed.DeleteResource(reqParams)
	if err != nil {
