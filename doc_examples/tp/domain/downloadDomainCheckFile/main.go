...
func main() {
	...
	reqParams := &domain.DownloadDomainCheckFileRequest{
		AccessToken: "#tpToken", // 文档中对应字段：access_token，实际使用时请替换成真实参数
	}

	resp, err := domain.DownloadDomainCheckFile(reqParams)
	if err != nil {
		...
		if resp.ContentType == utils.ConverterTypeStream {
			fileName := "./DownloadDomainCheckFileRequest.txt"
